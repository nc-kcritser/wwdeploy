...
if [ ! -f "${SCRIPT_DIR}/site-config.sh" ]; then
    echo "Error: site-config.sh not found in ${SCRIPT_DIR}. Cannot continue."
    exit 1
fi
...
for arg in "$@"; do
    case $arg in
        --firstrun|-f) FORCE_FIRSTRUN=true ;;
    esac
done
...
run_first() {
	...
	if [ ! -d "/opt/ohpc/pub/apps/dell/firmware/PowerEdge" ] || [ "$FORCE_FIRSTRUN" = true ]; then
		...
		console_info_msg "Updating SSH Settings ..."
        ...
        console_info_msg "Configuring Chrony for upstream NTP based on config file"
